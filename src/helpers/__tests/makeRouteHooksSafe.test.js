...
describe('makeRouteHooksSafe', () => {
  ...
  it('should work with JSX routes', () => {
    ...
    expect(routes[0].indexRoute.onEnter).toNotThrow(Error);
    expect(routes[0].childRoutes[1].onEnter).toNotThrow(Error);
    expect(routes[0].childRoutes[1].childRoutes[1].onEnter).toNotThrow(Error);
  });
  ...
  it('should work with JS routes', () => {
    ...
    expect(routes[0].indexRoute.onEnter).toNotThrow(Error);
    expect(routes[0].onEnter).toNotThrow(Error);
    expect(routes[0].childRoutes[1].onEnter).toNotThrow(Error);
  });
